...
export default function OnboardingPage() {
  ...
  const submit = async () => {
    setErr(null); if (!canSubmit) { setErr("Enter a valid 10-digit phone number"); return; } setBusy(true);
    try {
      ...
      if (res.status === "success") { if (SPA) setView('team'); else router.replace("/hunt/team"); }
      else { setErr(res.message || "Failed to submit details"); }
    } catch (e) { setErr(e instanceof Error ? e.message : "Failed to submit details"); }
  ...
  return (
    <div className="min-h-dvh ch-bg relative">
      ...
      <div className="relative ch-container ch-container-narrow pt-12 pb-36 safe-bottom">
        <header className="text-center"><h1 className="font-qurova ch-gradient-text ch-title ch-h1">cryptic hunt</h1></header>
        <div className="mt-3 text-center"><p className="leading-snug font-area ch-text ch-tagline">Please provide the following details to continue.</p></div>
        <section className="mt-8 grid gap-6">
          <div className="grid gap-3"><h2 className="font-qurova ch-gradient-text ch-h2">Gender</h2>
          ...
          <div className="grid gap-3"><h2 className="font-qurova ch-gradient-text ch-h2">Phone Number</h2>
            <div className="flex items-center gap-2">
              ...
              <input className="flex-1 h-11 rounded-xl px-4 bg-neutral-800 text-white outline-none font-area" placeholder="10-digit phone number" value={phone} onChange={(e) => setPhone(e.target.value.replace(/[^0-9]/g, ""))} maxLength={10} inputMode="numeric" />
            </div>
      ...
      <div className="fixed left-0 right-0 nav-safe-offset px-5" style={{ bottom: '2vh' }}>
        <div className="ch-card ch-glass rounded-2xl p-3">
          <button onClick={submit} disabled={!canSubmit || busy} className="w-full h-12 rounded-xl flex items-center justify-center gap-3 btn-ripple ch-btn"><span className="font-qurova">{busy ? 'Submitting…' : 'Continue'}</span></button>
        </div>
      </div>
    </div>
  );
