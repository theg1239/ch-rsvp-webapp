        ...
        {items.map((it) => {
          ...
          return it.external ? (
            <a key={it.href} href={it.href} target="_blank" rel="noopener noreferrer"
               className="relative flex flex-col items-center gap-1 px-3 py-1 hover:opacity-90">
              {content}
            </a>
          ) : (
            <Link key={it.href} href={it.href} className="relative flex flex-col items-center gap-1 px-3 py-1">
              {content}
            </Link>
          );
        })}
